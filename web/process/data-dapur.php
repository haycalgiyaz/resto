...
include "koneksi.php";
	...
	$query = mysqli_query($con, "SELECT * FROM tb_transaksi a JOIN tb_meja b ON a.id_meja=b.id_meja where a.kitchen_closed is NULL" );
	// $query = mysqli_query($con, "SELECT * FROM tb_transaksi");
	$data = array();
	while($row = mysqli_fetch_array($query)) {
			$id = $row['id_transaksi'];

			$det = mysqli_query($con, "SELECT * FROM tb_detail_transaksi a JOIN tb_menu b ON a.id_menu=b.id_menu where a.id_transaksi=$id and a.is_done in (0,1) and b.location='dapur'");
			$detail = array();
